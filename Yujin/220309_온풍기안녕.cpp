	...
	for (int i = 0; i < heater.size(); i++) {
			...
			while (!q.empty()) {
				...
				if (1 <= tx - 1 && tx - 1 <= r && 1 <= ty + 1 && ty + 1 <= c) {
					if (visited[tx - 1][ty + 1] != 1) {
						if (wall[tx][ty] != 0 && wall[tx - 1][ty] != 1) {
							visited[tx - 1][ty + 1] = 1;
							temp[tx - 1][ty + 1] += (t - 1);
				...
				if (1 <= tx + 1 && tx + 1 <= r && 1 <= ty + 1 && ty + 1 <= c) {
					if (visited[tx + 1][ty + 1] != 1) {
						if (wall[tx + 1][ty] != 0 && wall[tx + 1][ty] != 1){
							visited[tx + 1][ty + 1] = 1;
							temp[tx + 1][ty + 1] += (t - 1);
							q.push({ tx + 1, ty + 1, t - 1 });
						}
		...
		//왼쪽
			...
			while (!q.empty()) {
				...
				if (1 <= tx - 1 && tx - 1 <= r && 1 <= ty - 1 && ty - 1 <= c) {
					if (visited[tx - 1][ty - 1] != 1) {
						if (wall[tx][ty] != 0 && wall[tx - 1][ty-1] != 1) {
							visited[tx - 1][ty - 1] = 1;
							temp[tx - 1][ty - 1] += (t - 1);
							q.push({ tx - 1, ty - 1, t - 1 });
						}
				...
				if (1 <= tx + 1 && tx + 1 <= r && 1 <= ty - 1 && ty - 1 <= c) {
					if (visited[tx + 1][ty - 1] != 1) {
						if (wall[tx + 1][ty] != 0 && wall[tx + 1][ty-1] != 1) {
							visited[tx + 1][ty - 1] = 1;
							temp[tx + 1][ty - 1] += (t - 1);
							q.push({ tx + 1, ty - 1, t - 1 });
						}
		...
		//위쪽
			...
			while (!q.empty()) {
				...
				if (1 <= tx - 1 && tx - 1 <= r && 1 <= ty - 1 && ty - 1 <= c) {
					if (visited[tx - 1][ty - 1] != 1) {
						if (wall[tx][ty-1] != 1 && wall[tx][ty - 1] != 0) {
							visited[tx - 1][ty - 1] = 1;
							temp[tx - 1][ty - 1] += (t - 1);
							q.push({ tx - 1, ty - 1, t - 1 });
						}
				...
				if (1 <= tx -1 && tx - 1 <= r && 1 <= ty + 1 && ty + 1 <= c) {
					if (visited[tx - 1][ty + 1] != 1) {
						if (wall[tx][ty+1] != 0 && wall[tx][ty] != 1) {
							visited[tx - 1][ty + 1] = 1;
							temp[tx - 1][ty + 1] += (t - 1);
							q.push({ tx - 1, ty + 1, t - 1 });
						}
		...
		//아래쪽
			...
			while (!q.empty()) {
				...
				if (1 <= tx + 1 && tx + 1 <= r && 1 <= ty - 1 && ty - 1 <= c) {
					...
					if (visited[tx + 1][ty - 1] != 1) {
						//가는길이 안막혀있으면
						if (wall[tx+1][ty-1] != 0 && wall[tx][ty-1] != 1) {
							visited[tx + 1][ty - 1] = 1;
							temp[tx + 1][ty - 1] += (t - 1);
							q.push({ tx + 1, ty - 1, t - 1 });
						}
				...
				if (1 <= tx + 1 && tx + 1 <= r && 1 <= ty + 1 && ty + 1 <= c) {
					if (visited[tx + 1][ty + 1] != 1) {
						if (wall[tx + 1][ty+1] != 0 && wall[tx][ty] != 1) {
							visited[tx + 1][ty + 1] = 1;
							temp[tx + 1][ty + 1] += (t - 1);
							q.push({ tx + 1, ty + 1, t - 1 });
						}
		...
		for (int j = 1; j <= c; j++) {
			...
			for (int l = 1; l <= 4; l++) {
				...
				if (1 <= nx && nx <= r && 1 <= ny && ny <= c){
					//두 칸 사이에 벽이 없고
					if (l == 1 && wall[i][j] == 1) continue;
					if (l == 2 && wall[i][j - 1] == 1) continue;
					if (l == 3 && wall[i][j] == 0) continue;
					if (l == 4 && wall[i + 1][j] == 0) continue;
					//아직 계산되지 않았을 때
